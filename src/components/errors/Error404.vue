    ...
    <div class="h-screen w-screen relative overflow-hidden">
        ...
        <div class="absolute flex w-full top-0 h-full bottom-0 left-0 right-0 justify-center items-center">
            <div class="text-center">
                <h1 class="text-9xl text-black font-black uppercase dark:text-white">{{ $t('error.nope') }}</h1>
                <span
                    :class="[
                        'font-mono bg-slate-100 rounded-md border border-slate-200 px-2',
                        'dark:bg-slate-800 dark:border-slate-700 dark:text-white'
                    ]"
                >
                    {{ $t('error.error') }} 404
                </span>
                <p :class="['text-slate-400 intersect-once intersect:motion-translate-x-out-25 intersect:motion-opacity-in-0', 'dark:']">
                ...
                <div class="flex justify-center mt-10">
                    <router-link to="/" ref="link">
                        <ShimmerButton
                            class="shadow-2xl scale-100 hover:scale-105"
                            shimmer-size="2px"
                            shimmer-color="#000000BB"
                            background="#FFFFFF"
                        >
                            <span
                                :class="[
                                    'whitespace-pre-wrap text-center font-medium leading-none tracking-tight text-white lg:text-lg dark:from-white dark:to-slate-900/10 text-sm',
                                    'dark:text-black'
                                ]"
                            >
                                Take me home
